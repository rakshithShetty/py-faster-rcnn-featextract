...
This codebase is a clone of py-faster-rcnn from Ross Girshik (https://github.com/rbgirshick/py-faster-rcnn) with additional scripts.
I have added support to few more databases and written scripts to convert faster rcnn output to feature vectors used for image captioning.
These are the auxillary features used in image captioning papers:
...
###Example Usage to extract features from faster rcnn output json file

./tools/dumpLocFeatures.py <FasterRCNN output json file> -o <Name of the feature file > --imdb coco_2015_test-yle --gridtype rect --grid 3 --labels /projects/databases/yle2016/labelsMod.txt

### To dump the faster rcnn object detection outputs:
./tools/test_net.py --gpu 0 --def models/coco/VGG16/faster_rcnn_end2end/test.prototxt --net trainedModels/coco80Cls_vgg16_faster_rcnn_iter_290000.caffemodel --imdb coco_2015_test-yle  --cfg experiments/cfgs/faster_rcnn_end2end.yml --comp --set EXP_DIR featExtract



#------------------ Original faster rcnn readme ------------------
### Disclaimer
